<aside
    ...
    @php
  $isDashboard = request()->routeIs('dashboard.monitoring');
  $isKrono = request()->routeIs('kronologi');
@endphp
    <hr class="h-px mt-0 bg-transparent bg-gradient-to-r from-transparent via-black/40 to-transparent" />
    <div
        class="
      grow basis-full
      h-full                  /* isi mengambil full tinggi aside */
      overflow-y-auto         /* konten sidebar yang scroll */
      px-4
    ">
        ...
        <ul class="flex flex-col pl-0 mb-0">
            <li class="mt-0.5 w-full">
                <a @class([
                    ...
                    // Active state
                    'transition-colors text-slate-700 shadow-soft-xl rounded-lg font-semibold' => $isDashboard,
                    // Inactive state
                    'hover:bg-slate-100' => ! $isDashboard,
                  ]) href="{{ route('dashboard.monitoring') }}">
                    
                    <div
                    @class([
                        'mr-2 flex h-8 w-8 items-center justify-center rounded-lg xl:p-2.5',
                        // Active icon bg
                        'bg-gradient-to-tl from-blue-600 to-cyan-400 shadow-soft-2xl' => $isDashboard,
                        // Inactive icon bg
                        'bg-white shadow-soft-2xl' => ! $isDashboard,
                    ])>
            ...
            @endif
            {{-- Khusus role = 'monitoring' --}}
            @if (Auth::check() && Auth::user()->role === 'monitoring')
                <li class="w-full mt-4">
                    <h6 class="pl-6 ml-2 font-bold leading-tight uppercase text-xs opacity-60">Fitur TLS</h6>
                </li>

                <li class="mt-0.5 w-full">
                    <a @class([
                        'py-2.7 text-sm ease-nav-brand my-0 mx-4 flex items-center whitespace-nowrap px-4 transition-colors',
                        'transition-colors text-slate-700 shadow-soft-xl rounded-lg font-semibold' => $isKrono,
                        'hover:bg-slate-100' => ! $isKrono,
                      ]) href="{{ route('kronologi') }}">

                        <div
                        @class([
                            'mr-2 flex h-8 w-8 items-center justify-center rounded-lg xl:p-2.5',
                            'bg-gradient-to-tl from-blue-600 to-cyan-400 shadow-soft-2xl' => $isKrono,
                            'bg-white shadow-soft-2xl' => ! $isKrono,
                        ])>
